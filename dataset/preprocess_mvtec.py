...
from dataset.config import   mvtec_root_dir,  exp_path
...
out_root_dir = '../data/defect_392/mvtec'
    ...
    for dset in ['train','test']:
        for category in ['ok','ng','false_ng']:
            ...
            for i, image_path in tqdm(enumerate(path_list)):
                ...
                crop_image = cv2.resize(origin_img,(resize[0],resize[-1]))[center[0]-crop_size[0]//2:center[0]+crop_size[0]//2,
                             center[1]-crop_size[1]//2:center[1]+crop_size[1]//2]

                crop_pha_image = cv2.resize(origin_pha,(resize[0],resize[-1]))[center[0]-crop_size[0]//2:center[0]+crop_size[0]//2,
                             center[1]-crop_size[1]//2:center[1]+crop_size[1]//2]

                crop_pha_image = cv2.cvtColor(crop_pha_image, cv2.COLOR_BGR2GRAY)
                if dset == 'train' and category == 'false_ng':
        #             if single_dataset in  ['bottle', 'cable', 'capsule','hazelnut', 'metal_nut','pill', 'screw',
        ...
        #                 fg[fg <= 0.5] = 0
        #             else:
                    fg = np.ones_like(crop_pha_image)
                    while True:
                        crop_image_, m = generate_anomaly(img=crop_image, target_foreground_mask=fg)
